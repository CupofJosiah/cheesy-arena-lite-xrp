...
func TestPlayoffTournamentUpdateMatches(t *testing.T) {
	...
	matches, _ := database.GetMatchesByType(model.Playoff, true)
	assert.Equal(t, game.TeamId("102"), matches[0].Red1)
	assert.Equal(t, game.TeamId("101"), matches[0].Red2)
	assert.Equal(t, game.TeamId("402"), matches[0].Blue1)
	assert.Equal(t, game.TeamId("401"), matches[0].Blue2)

	matches[0].Status = game.BlueWonMatch
	err = database.UpdateMatch(&matches[0])
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(1, [game.TeamsPerAlliance]game.TeamId{"103", "102"})
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(4, [game.TeamsPerAlliance]game.TeamId{"404", "405"})
	assert.Nil(t, err)
	...
	assert.Equal(t, game.TeamId("103"), matches[2].Red1)
	assert.Equal(t, game.TeamId("102"), matches[2].Red2)
	assert.Equal(t, game.TeamId("404"), matches[2].Blue1)
	assert.Equal(t, game.TeamId("405"), matches[2].Blue2)
	assert.Equal(t, game.MatchScheduled, matches[4].Status)
	assert.Equal(t, game.TeamId("103"), matches[4].Red1)
	assert.Equal(t, game.TeamId("102"), matches[4].Red2)
	assert.Equal(t, game.TeamId("404"), matches[4].Blue1)
	assert.Equal(t, game.TeamId("405"), matches[4].Blue2)
	...
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(4, [game.TeamsPerAlliance]game.TeamId{"403", "402"})
	assert.Nil(t, err)
	...
	assert.Equal(t, game.TeamId("403"), matches[6].Red1)
	assert.Equal(t, game.TeamId("402"), matches[6].Red2)
	assert.Equal(t, game.TeamId("302"), matches[6].Blue1)
	assert.Equal(t, game.TeamId("301"), matches[6].Blue2)
	...
	assert.Equal(t, game.TeamId(""), matches[6].Red1)
	assert.Equal(t, game.TeamId(""), matches[6].Red2)
	assert.Equal(t, game.TeamId(""), matches[6].Blue1)
	assert.Equal(t, game.TeamId(""), matches[6].Blue2)
}
