...
func TestPlayoffTournamentUpdateMatches(t *testing.T) {
	...
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(1, [game.TeamsPerAlliance]int{103, 102})
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(4, [game.TeamsPerAlliance]int{404, 405})
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	err = database.UpdateAllianceFromMatch(4, [game.TeamsPerAlliance]int{403, 402})
	assert.Nil(t, err)
