...
func (signs *TeamSigns) Update(arena *Arena) {
	...
	case TeleopPeriod:
		countdownSec = game.MatchTiming.AutoDurationSec + game.MatchTiming.PauseDurationSec +
			game.MatchTiming.TeleopDurationSec - matchTimeSec
	case TimeoutActive:
	...
	redInMatchTeamRearText := generateInMatchTeamRearText(arena, true)
	redInMatchTimerRearText := generateInMatchTimerRearText(arena, true, rearCountdown)
	blueInMatchTeamRearText := generateInMatchTeamRearText(arena, false)
...
func generateInMatchTeamRearText(arena *Arena, isRed bool) string {
	allianceScores := generateTeamSignAllianceScores(arena, isRed)
	return formatTeamSignRearText(allianceScores)
}
...
func generateTeamSignAllianceScores(arena *Arena, isRed bool) string {
	...
	return fmt.Sprintf(
		formatString,
		scoreSummary.Score-scoreSummary.PostMatchPoints,
		opponentScoreSummary.Score-opponentScoreSummary.PostMatchPoints,
	)
}
...
func generateTimerTexts(arena *Arena, countdown, inMatchRearText string) (string, color.RGBA, string) {
	...
	if arena.AllianceStationDisplayMode == "logo" {
		frontText = currentYearTeamSignText()
...
func currentYearTeamSignText() string {
	return fmt.Sprintf("%5d", time.Now().Year())
}
