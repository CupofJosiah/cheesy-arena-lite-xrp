...
func TestSortRankings(t *testing.T) {
	...
	sort.Sort(rankings)
	assert.Equal(t, []TeamId{"10", "8", "6", "4", "2", "1", "3", "5", "7", "9"}, rankingTeamIds(rankings))
	...
	sort.Sort(rankings)
	assert.Equal(t, []TeamId{"2", "3", "1"}, rankingTeamIds(rankings))
}

func rankingTeamIds(rankings Rankings) []TeamId {
	teamIds := make([]TeamId, len(rankings))
