...
func TestAllianceSelection(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		web.arena.Database.CreateRanking(&game.Ranking{TeamId: teamId(100 + i), Rank: i})
	}
	...
	assert.Equal(t, 303, recorder.Code)
	assert.Equal(t, game.TeamId("110"), web.arena.AllianceSelectionAlliances[0].TeamIds[0])
	recorder = web.getHttpResponse("/alliance_selection")
	assert.Contains(t, recorder.Body.String(), "\"110\"")
	assert.NotContains(t, recorder.Body.String(), ">110<")
	...
	assert.Equal(t, 303, recorder.Code)
	assert.Equal(t, game.TeamId("101"), web.arena.AllianceSelectionAlliances[0].TeamIds[0])
	assert.Equal(t, game.TeamId("102"), web.arena.AllianceSelectionAlliances[0].TeamIds[1])
	assert.Equal(t, game.TeamId("103"), web.arena.AllianceSelectionAlliances[1].TeamIds[0])
	recorder = web.getHttpResponse("/alliance_selection")
	assert.Contains(t, recorder.Body.String(), ">110<")
	...
	if assert.Equal(t, 3, len(alliances)) {
		assert.Equal(t, game.TeamId("101"), alliances[0].TeamIds[0])
		assert.Equal(t, game.TeamId("105"), alliances[1].TeamIds[1])
		assert.Equal(t, game.TeamId("109"), alliances[2].TeamIds[2])

		// Check that the initial lineup is populated correctly.
		assert.Equal(t, game.TeamId("102"), alliances[0].Lineup[0])
		assert.Equal(t, game.TeamId("101"), alliances[0].Lineup[1])
	}
...
func TestAllianceSelectionErrors(t *testing.T) {
	...
	// Select invalid teams. A team number may contain letters, but not punctuation.
	recorder = web.postHttpResponse("/alliance_selection", "selection0_0=as-df")
	assert.Equal(t, 200, recorder.Code)
